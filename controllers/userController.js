...
const signupUser = async (req, res) => {
    ...
    try { 
        const user = await User.signup(email_id, password, outlet_division,role,code,outlet_name)
...
const bulkSignupUsers = async (req, res) => {
    ...
    try {
      const bulkPromises = users.map(async (user) => {
        const { email_id, password, outlet_division, role, code, outlet_name } = user;
        try {
          return await User.signup(email_id, password, outlet_division, role, code, outlet_name);
        } catch (err) {
